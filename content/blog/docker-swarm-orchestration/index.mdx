...
참고한 기존 블로그 글에서는 registry 컨테이너가 별도로 존재했지만 실습 결과 딱히 필요성을 느끼지 못해 삭제하였다. (docker registy는 docker hub를 대체하는 로컬서버정도로 이해하면 될 것 같다.)
...
만약 worker node를 포함해 4개의 node가 나온다면 클러스터를 만드는데 성공한 것이다.
...
이를 테스트 해보기 위해 [hashicorp/http-echo](https://hub.docker.com/r/hashicorp/http-echo/) 이미지를 사용해보자
...
[스웜(swarm)을 이용한 도커 컨테이너 배포\_1](https://cornswrold.tistory.com/512?category=930033)  
