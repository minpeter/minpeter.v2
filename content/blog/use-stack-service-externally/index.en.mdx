...
Finally, if you access [localhost:8000](http://localhost:8000) in your browser locally, you'll see the message `hello, flask!`. The reason we access port 8000 even though we forwarded to port 80 in the proxy server is because we set the manager node's port to 8000:80 when we created the cluster containers.

## References

[Deploying Docker Containers Using Swarm_3 (Using Services from Outside the Swarm Cluster)](https://cornswrold.tistory.com/516?category=930033) [docker docs](https://docs.docker.com/engine/swarm/ingress/)
