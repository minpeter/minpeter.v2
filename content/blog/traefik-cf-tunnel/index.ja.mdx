...
2. 「_.yourdomain.xyz」（ルートワイルドカードレコード）の代わりに「_.str.yourdomain.xyz」のような連結されたサブドメインワイルドカードを処理できません。これはCloudflareで証明書を発行してくれないためで、tunnelを使用する限り解決方法がありません。再びポートフォワーディングをしてDNS only + 自己証明書方式で解決する必要があります。
