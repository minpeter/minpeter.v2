...
| TUNNEL_TOKEN | 위에서 발급 받았던 cf tunnel의 토큰입니다. 그대로 붙여 넣어줍시다. |
| --- | --- |
...
2. “_.yourdomain.xyz” (root wildcard record) 대신 “_.str.yourdomain.xyz” 같은 연결된 서브도메인 wildcard를 처리할 수 없습니다. 이는 cloudflare에서 인증서 발급을 안해주기 때문이고, tunnel를 사용하는 한 해결방법이 없습니다. 다시 포트포워딩을 하여 DNS only + 자체 인증서 방식으로 해결하여야 합니다.
