...
글 안에서만 디자인이 끝나지 않도록 링크를 공유할 때 보이는 오픈 그래프 이미지도 새로 만들었다. 이제 모든 글은 하나의 정적인 썸네일을 돌려 쓰는 대신, 제목과 발행일을 읽어 1200×630 이미지로 자동 생성한다. 고양이 마크와 `MINPETER.COM`, 얇은 그리드와 흑백 색감도 사이트의 첫 화면에서 그대로 이어진다.

한글·영어·일본어에는 Fusion Pixel 10px Proportional을 사용했다. 같은 글의 번역은 제목 크기의 기준을 공유하고, 라틴 문자와 CJK 문자의 실제 높이와 기준선 차이는 글자 단위로 보정했다. 한 문장 안에 여러 언어가 섞여도 높이가 갑자기 달라 보이지 않게 하기 위해서다. 아래 이미지는 이 글의 제목과 날짜로 실제 생성한 결과다.

![이 글의 제목과 발행일로 자동 생성한 오픈 그래프 이미지](./images/open-graph-ko.png)
