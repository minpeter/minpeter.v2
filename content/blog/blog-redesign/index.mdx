...
글 안에서만 디자인이 끝나지 않도록 링크를 공유할 때 보이는 오픈 그래프 이미지도 새로 만들었다. 이제 모든 글은 하나의 정적인 썸네일을 돌려 쓰는 대신, 제목과 발행일을 읽어 1200×630 이미지로 자동 생성한다. 고양이 마크와 `MINPETER.UK`, 얇은 그리드와 흑백 색감도 사이트의 첫 화면에서 그대로 이어진다.
