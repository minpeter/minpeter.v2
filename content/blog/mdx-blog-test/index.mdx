...
import { ModCodeBlock } from "@/components/mod-code-block";
...
우선 인라인 코드 블럭 테스트 `console.log("Hello, World!")` 를 해보자. 다음은 일반 코드 블럭 테스트
...
ip.minpeter.com 서버를 이용한 독자 아이피 조회 컴포넌트
...
export function Ip() {
  ...
  useEffect(() => {
    fetch("https://ip.minpeter.com/ip").then((res) =>
...
위에서 ModCodeBlock를 이용해 `vpc-id`, `cidr-block`, `availability-zone`를 변수로 사용할 수 있다. 이때 사용되는 형식은 이러하다.
