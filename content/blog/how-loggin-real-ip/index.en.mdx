...
You probably have at least one backend you've built yourself. In my case, I had [api.tmpf.me](https://api.tmpf.me), but there was one thing I felt was lacking.

That was `user access logging..` - the work of recording users who access the service. It might not be strictly necessary, but let's assume it is as we read this article.
...
As shown in the image below, `211.104.53.76` is the client's IP. (My IP while writing this article) ![It's a Starbucks IP.. let's not harass it.](./images/Untitled.png)
...
This header is a ~~de facto~~ standard header created to pass the RemoteAddr (which would be lost when passing through proxies) to services behind the proxy. The following screenshot shows a whoami service running behind 2 reverse proxies.
...
The IPs differ from the image above, but it has the following structure: ![](./images/Untitled-2024-02-13-1709.svg)
