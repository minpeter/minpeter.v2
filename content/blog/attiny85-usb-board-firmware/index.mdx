...
따라서 정품, 카피 구분없이 펌웨어를 2.6 버전으로 업그레이드 할 것이다.
...
엥? 그럼 usb는 왜 있을까?  
attiny85 칩셋이 v-usb를 지원... 아니 atmel사의 거의 모든 AVR이 v-usb 기능을 지원하기 때문에 존재한다.  
...
HelloWorld를 두 번 입력하고 led를 10번 깜빡이는 코드다. 업로드를 진행해보자.
...
마치 사람이 직접 입력하는 것으로 받아들이고 커서의 위치에 HelloWorld를 두 번 적을 것이다.  
