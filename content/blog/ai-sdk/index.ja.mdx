...
VercelはNext.jsを開発した会社で、開発者がLLM（大規模言語モデル）をウェブサイトに簡単に統合できるよう支援する[AI SDK](https://sdk.vercel.ai/)を提供しています。このSDKは別途のNode.jsサーバーを使用するほとんどのフロントエンドフレームワークで適用可能であり、特にサーバーを標準で提供するNext.jsと一緒に使用する際に非常に便利です。
...
AI SDKを使用するにはLLMモデルを提供する**provider**が必要です。この例では[FriendliAI](https://friendli.ai/)を使用して設定を進めます。他のAI providerを使用したい場合は[Vercel AI SDK providers ドキュメント](https://sdk.vercel.ai/providers/ai-sdk-providers)を参照してください。
...
これでFriendliAIの**Llama 3.1 70b**モデルを$5クレジット分無料で使用できます。Llama 3.1モデルはMetaの最新LLMで優れた性能を誇ります。
