...
그 중 거의 항상 언급되는 말이 실제로 운용할 때는 GlusterFS 같은 서비스를 이용해야한다는 것이였다.  
근데 GlusterFS가 성능이 좋을 지언정 최소 3개의 노드를 운용해야되다는 단점이 있었고 (적어도 나의 경우에는) Proxmox에 남은 자원이 없었다.  
...
단 openEBS의 자세한 내용이나 아직 pv, pvc에 대한 내용을 정리하지 않아서 현재 상황에 이게 적절한 설정인건지 알 수 없다.  
