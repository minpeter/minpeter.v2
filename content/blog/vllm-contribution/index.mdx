---
title: 첫 vLLM 오픈소스 기여
description: HermesToolParser 개선을 통한 vLLM 업스트림 기여 경험
date: 2025-08-19
---

## 개요

vLLM은 LLM 추론을 위한 대표적인 오픈소스 프로젝트입니다. 이번에 HermesToolParser의 개선 사항을 [PR #16890](https://github.com/vllm-project/vllm/pull/16890)으로 기여하여 메인 브랜치에 머지되었습니다.

이 글에서는 기여하게 된 배경과 구현 내용, 그리고 오픈소스 기여 과정에서 배운 점을 공유합니다.

## 문제 상황

vLLM의 기존 HermesToolParser는 `<tool_call>`과 `</tool_call>` 태그가 **별도의 special token**으로 정의된 모델에서만 정상 동작했습니다.

하지만 Llama와 같은 모델을 Hermes 포맷으로 fine-tuning하면, 이 태그들이 special token이 아닌 일반 텍스트로 토큰화됩니다. 예를 들어:
...
이 문제를 해결하기 위해 [hermes-llama-parse](https://github.com/minpeter/hermes-llama-parse) 레포에서 먼저 프로토타입을 구현했습니다.
...
테스트를 위해 직접 fine-tuning한 모델 [minpeter/m-1b-v1-sf-xlam-09](https://huggingface.co/minpeter/m-1b-v1-sf-xlam-09)를 사용한 e2e 테스트를 작성했습니다.
...
- [테스트 모델 (HuggingFace)](https://huggingface.co/minpeter/m-1b-v1-sf-xlam-09)
