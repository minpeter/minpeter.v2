...
근데 저래도 앞으로 github에서 생성하는 레포에 한해 main으로 설정하는 것이기에 이미 master로 설정되어 있는 레포들은 일일히 변경해줘야 한다.
...
git 버전이 2.28 이후의 버전에서는 기본 브랜치 설정을 변경하기 위한 `init.defaultBranch` 설정이 새로 도입됐다.
