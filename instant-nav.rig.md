...
- BUILD: `EXPOSE_TESTING_API=1 pnpm build` then Playwright webServer runs `next start -p 8200`
- EXPOSE: `experimental.exposeTestingApiInProductionBuild` when `process.env.EXPOSE_TESTING_API === "1"` (`next.config.mts`)
- RUN: `pnpm test:e2e` / `pnpm test:e2e:instant` → Playwright against `http://127.0.0.1:8200` (or `BASE_URL`)
...
- LOOP: build with EXPOSE → Playwright starts `next start` → tests; fully local/agent-drivable
- LIVENESS: n/a for local build + Playwright webServer (artifact is the one just built)
- WALLS:
