...
const fusionPixelFontUrls = {
  ja: new URL(
    "../../../../../shared/font.AritaBuri/FusionPixel10Proportional-ja.woff",
    import.meta.url
  ),
  ko: new URL(
    "../../../../../shared/font.AritaBuri/FusionPixel10Proportional-ko.woff",
    import.meta.url
  ),
};
...
const getTitleVisualWidth = (title: string) =>
  [...title].reduce(
    (total, character) => total + getCharacterWidth(character),
    0
  );

const getTitleSize = (visualWidth: number) => {
...
export const GET = async (
  ...
  const title = (post.data.title ?? siteConfig.title).toLocaleUpperCase(locale);
  const localizedTitleWidths = ["ko", "en", "ja"].flatMap((localizedLocale) => {
    ...
    return [getTitleVisualWidth(localizedTitle)];
  });
  const titleSize = getTitleSize(Math.max(...localizedTitleWidths));
  const fusionPixelFontData = await readFile(
    locale === "ja" ? fusionPixelFontUrls.ja : fusionPixelFontUrls.ko
  );

  return new ImageResponse(
    <OgImage publishedIso={publishedIso} title={title} titleSize={titleSize} />,
    {
      ...size,
      fonts: [
        {
