...
function GeneratedModelIcon({ generatedBy }: { generatedBy: string }) {
  const className = "shrink-0 text-black dark:text-white";

  if (/kimi|moonshot/i.test(generatedBy)) {
    return <KimiIcon className={className} size={16} />;
  }
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      <div className="flex items-center gap-1.5 text-foreground">
        {generatedBy ? (
          <GeneratedModelIcon generatedBy={generatedBy} />
        ) : (
          <ClaudeIcon size={16} />
        )}
        <span className="text-sm">
          {generatedBy
            ? t("generatedBy", { model: generatedBy })
            : t("translatedBy")}
        </span>
      </div>
